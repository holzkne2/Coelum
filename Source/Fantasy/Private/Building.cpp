...
{
	for (int i = 0; i < MeshComponent->GetNumSections(); ++i) {
	...
	if (FillBottom) {
		...
		for (int i = 0; i < vertices.Num(); ++i) {
			...
			vertices[i] = GetTransform().InverseTransformPosition(vertices[i]);
		}

		MeshComponent->CreateMeshSection(3, vertices, triangles, normals, TArray<FVector2D>(), TArray<FVector2D>(), TArray<FVector2D>(), TArray<FVector2D>(), TArray<FColor>(), TArray<FProcMeshTangent>(), true);
		MeshComponent->SetMaterial(3, TempMat);
	}
